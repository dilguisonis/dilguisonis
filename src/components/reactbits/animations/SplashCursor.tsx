...
export default function SplashCursor({
  ...
  colors = ['#00fff5', '#ff00ff', '#00ff41', '#ffb000']
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    const halfFloatTexType = isWebGL2 ? gl.HALF_FLOAT : (halfFloat && (halfFloat as { HALF_FLOAT_OES: number }).HALF_FLOAT_OES) || 0;
    ...
    function compileShader(type: number, source: string, keywords: string[] | null = null): WebGLShader | null {
      ...
      const shaderSource = keywordsString + source;
    ...
    function render(target: FBO | null) {
      ...
      if (displayProgram.uniforms.texelSize) gl.uniform2f(displayProgram.uniforms.texelSize, 1 / width, 1 / height);
      if (displayProgram.uniforms.uTexture) gl.uniform1i(displayProgram.uniforms.uTexture, dye.read.attach(0));
      blit(target, false);
    }
  ...
  }, [isMobile, prefersReducedMotion, SIM_RESOLUTION, DYE_RESOLUTION, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE, PRESSURE_ITERATIONS, CURL, SPLAT_RADIUS, SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, colors]);
